...
#
# scripts/test.sh runs the full suite (pytest + vitest, see docs/MERGE_SPEC.md Phase 8) —
# use that instead of `.venv/bin/pytest -q` alone so a change that only breaks the
# front-end jsdom tests still blocks the turn.
TEST_CMD="$CLAUDE_PROJECT_DIR/scripts/test.sh"
